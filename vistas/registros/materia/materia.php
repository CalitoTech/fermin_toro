...
if ($alert) {
    switch ($alert) {
        case 'success':
            $alerta = Notificaciones::exito("El materia se creó correctamente.");
            break;
        case 'actualizar':
            $alerta = Notificaciones::exito("El materia se actualizó correctamente.");
            break;
        case 'deleted':
            $alerta = Notificaciones::exito("El materia se eliminó correctamente.");
            break;
        case 'dependency_error':
            $alerta = Notificaciones::advertencia("No se puede eliminar el materia porque está siendo utilizado por una o más personas.");
            break;
        case 'error':
            $alerta = Notificaciones::advertencia("Este materia ya existe, verifique por favor.");
            break;
        default:
            $alerta = null;
    }
...
<head>
    <title>UECFT Araure - Materias</title>
</head>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <i class='bx bxs-user-detail'></i> Gestión de Materias
                        </div>
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                                ...
                                <!-- Nuevo Materia a la derecha -->
                                <a href="nuevo_materia.php" class="btn btn-danger d-flex align-items-center">
                                    <i class='bx bx-plus-medical me-1'></i> Nuevo Materia
                                </a>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle" id="tabla-materias">
                                    ...
                                    <tbody id="table-body">
                                        ...
                                        foreach ($materias as $user): ?>
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($user['materia']) ?></td>
                                                <td>
                                                    <a href="editar_materia.php?id=<?= $user['IdMateria'] ?>" class="btn btn-sm btn-outline-primary me-1">
                                                        <i class='bx bxs-edit'></i>
                                                    </a>
                                                    <button class="btn btn-sm btn-outline-danger" onclick="confirmDelete(<?= $user['IdMateria'] ?>)">
...
<script>
    // === DATOS GLOBALES ===
    let allData = <?= json_encode($materias) ?>;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const config = {
            tablaId: 'tabla-materias',  // Coincide con tu HTML
        ...
        window.tablaMaterias = new TablaDinamica(config);
    });

    // === FUNCIONES ===
    function confirmDelete(id) {
        Swal.fire({
            title: "¿Está seguro que desea eliminar este materia?",
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                window.location.href = '../../../controladores/MateriaController.php?action=eliminar&id=' + id;
            } else if (result.isDenied) {
                Swal.fire("No se eliminó el materia", "", "info");
            }
        });
    }
    ...
    window.imprimirLista = function() {
        ...
        generarReporteImprimible(
            'REPORTE DE MATERIAS DEL SISTEMA',
            '#tabla-materias',
            {
