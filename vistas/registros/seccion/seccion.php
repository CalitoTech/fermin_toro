...
if ($alert) {
    switch ($alert) {
        case 'success':
            $alerta = Notificaciones::exito("El seccion se creó correctamente.");
            break;
        case 'actualizar':
            $alerta = Notificaciones::exito("El seccion se actualizó correctamente.");
            break;
        case 'deleted':
            $alerta = Notificaciones::exito("El seccion se eliminó correctamente.");
            break;
        case 'dependency_error':
            $alerta = Notificaciones::advertencia("No se puede eliminar el seccion porque está siendo utilizado por una o más personas.");
            break;
        case 'error':
            $alerta = Notificaciones::advertencia("Este seccion ya existe, verifique por favor.");
            break;
        default:
            $alerta = null;
    }
...
<head>
    <title>UECFT Araure - Secciones</title>
</head>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <i class='bx bxs-user-detail'></i> Gestión de Secciones
                        </div>
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                                ...
                                <!-- Nuevo Seccion a la derecha -->
                                <a href="nuevo_seccion.php" class="btn btn-danger d-flex align-items-center">
                                    <i class='bx bx-plus-medical me-1'></i> Nuevo Seccion
                                </a>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle" id="tabla-secciones">
                                    ...
                                    <tbody id="table-body">
                                        ...
                                        foreach ($secciones as $user): ?>
                                            <tr>
                                                <td><?= htmlspecialchars($user['IdSeccion']) ?></td>
                                                <td><?= htmlspecialchars($user['seccion']) ?></td>
                                                <td>
                                                    <a href="editar_seccion.php?id=<?= $user['IdSeccion'] ?>" class="btn btn-sm btn-outline-primary me-1">
                                                        <i class='bx bxs-edit'></i>
                                                    </a>
                                                    <button class="btn btn-sm btn-outline-danger" onclick="confirmDelete(<?= $user['IdSeccion'] ?>)">
...
<script>
    // === DATOS GLOBALES ===
    let allData = <?= json_encode($secciones) ?>;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const config = {
            tablaId: 'tabla-secciones',  // Coincide con tu HTML
            ...
            idField: 'IdSeccion',
            columns: [
                { label: 'ID', key: 'IdSeccion' },
                { label: 'Seccion', key: 'seccion' }
            ],
            acciones: [
                {
                    url: 'editar_seccion.php?id={id}',
        ...
        window.tablaSecciones = new TablaDinamica(config);
    });

    // === FUNCIONES ===
    function confirmDelete(id) {
        Swal.fire({
            title: "¿Está seguro que desea eliminar este seccion?",
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                window.location.href = '../../../controladores/SeccionController.php?action=eliminar&id=' + id;
            } else if (result.isDenied) {
                Swal.fire("No se eliminó el seccion", "", "info");
            }
        });
    }
