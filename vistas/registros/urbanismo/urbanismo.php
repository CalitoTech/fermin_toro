...
<head>
    <title>UECFT Araure - Urbanismos</title>
</head>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <i class='bx bxs-user-detail'></i> Gestión de Urbanismos
                        </div>
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                                ...
                                <!-- Nuevo Urbanismo a la derecha -->
                                <a href="nuevo_urbanismo.php" class="btn btn-danger d-flex align-items-center">
                                    <i class='bx bx-plus-medical me-1'></i> Nuevo Urbanismo
                                </a>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle" id="tabla-urbanismos">
                                    ...
                                    <tbody id="table-body">
                                        ...
                                        $query = "SELECT IdUrbanismo, urbanismo
                                                  FROM urbanismo
                                                  ORDER BY urbanismo ASC";
                                        $stmt = $conexion->prepare($query);
                                        $stmt->execute();
                                        $urbanismos = $stmt->fetchAll(PDO::FETCH_ASSOC);

                                        foreach ($urbanismos as $user): ?>
                                            <tr>
                                                <td><?= htmlspecialchars($user['IdUrbanismo']) ?></td>
                                                <td><?= htmlspecialchars($user['urbanismo']) ?></td>
                                                <td>
                                                    <a href="editar_urbanismo.php?id=<?= $user['IdUrbanismo'] ?>" class="btn btn-sm btn-outline-primary me-1">
                                                        <i class='bx bxs-edit'></i>
                                                    </a>
                                                    <button class="btn btn-sm btn-outline-danger" onclick="confirmDelete(<?= $user['IdUrbanismo'] ?>)">
...
<script>
    // === DATOS GLOBALES ===
    let allData = <?= json_encode($urbanismos) ?>;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const config = {
            tablaId: 'tabla-urbanismos',  // Coincide con tu HTML
            ...
            idField: 'IdUrbanismo',
            columns: [
                { label: 'ID', key: 'IdUrbanismo' },
                { label: 'Urbanismo', key: 'urbanismo' }
            ],
            acciones: [
                {
                    url: 'editar_urbanismo.php?id={id}',
        ...
        window.tablaUrbanismos = new TablaDinamica(config);
    });

    // === FUNCIONES ===
    function confirmDelete(id) {
        Swal.fire({
            title: "¿Está seguro que desea eliminar este urbanismo?",
            showDenyButton: true,
